  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{session.title || 'Research Session'}</h1>
          <p className="text-sm text-gray-500 mt-1 line-clamp-2 sm:line-clamp-none">{session.research_objective}</p>
        ...
        <div className="flex flex-wrap gap-2">
          {isRunning && (
            <>
              <button onClick={fetchSession} className="p-2.5 border border-gray-300 rounded-lg hover:bg-gray-50">
                <RefreshCw className="h-4 w-4" />
              </button>
              <button onClick={handleCancel} className="flex items-center gap-1 px-3 py-2.5 bg-red-50 text-red-600 rounded-lg text-sm hover:bg-red-100">
          ...
          {session.status === 'completed' && (
            <div className="flex flex-wrap gap-1">
              {['markdown', 'json', 'csv'].map((fmt) => (
                <button key={fmt} onClick={() => handleExport(fmt)} className="flex items-center gap-1 px-3 py-2.5 border border-gray-300 rounded-lg text-sm hover:bg-gray-50">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4">
          <div className="flex items-center gap-1.5 text-gray-500 mb-1"><BookOpen className="h-3.5 w-3.5 sm:h-4 sm:w-4" /><span className="text-xs">Papers Found</span></div>
          <p className="text-xl sm:text-2xl font-bold">{session.total_papers_discovered}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4">
          <div className="flex items-center gap-1.5 text-gray-500 mb-1"><Activity className="h-3.5 w-3.5 sm:h-4 sm:w-4" /><span className="text-xs">Evaluated</span></div>
          <p className="text-xl sm:text-2xl font-bold">{session.total_papers_evaluated}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4">
          <div className="flex items-center gap-1.5 text-gray-500 mb-1"><FileText className="h-3.5 w-3.5 sm:h-4 sm:w-4" /><span className="text-xs">Avg AGI Score</span></div>
          <p className="text-xl sm:text-2xl font-bold">{session.avg_agi_score?.toFixed(1) ?? '--'}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4">
          <div className="flex items-center gap-1.5 text-gray-500 mb-1"><Clock className="h-3.5 w-3.5 sm:h-4 sm:w-4" /><span className="text-xs">Duration</span></div>
          <p className="text-xl sm:text-2xl font-bold">{session.processing_time_seconds ? `${session.processing_time_seconds.toFixed(0)}s` : '--'}</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-5">
          ...
          <div className="space-y-2">
            {session.papers.map((paper) => (
              <Link key={paper.id} to={`/papers/${paper.id}`} className="flex items-center justify-between p-2 sm:p-3 rounded-lg hover:bg-gray-50">
      ...
      {logs.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200">
          ...
          {showLogs && (
            <div className="px-3 sm:px-5 pb-3 sm:pb-5 max-h-96 overflow-y-auto space-y-1">
              {logs.map((log) => (
                <div key={log.id} className="text-xs py-1.5 border-b border-gray-100">
                  ...
                  <p className="text-gray-800 mt-0.5 break-words">{log.message}</p>
                </div>
              ))}
            </div>
          )}
