  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Scheduled Research</h1>
          <p className="text-gray-500">Automated recurring research jobs</p>
        </div>
        <button
            ...
            <div key={job.id} className="bg-white rounded-xl border border-gray-200 p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">{job.research_objective}</p>
                  <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                    <span>Frequency: {job.frequency}</span>
                    <span>Max Papers: {job.max_papers}</span>
                    <span>Total Runs: {job.total_runs}</span>
                ...
                <div className="flex gap-2">
                  <button onClick={() => toggleJob(job.id)} className="p-2 border border-gray-300 rounded-lg hover:bg-gray-50">
                    <Power className={`h-4 w-4 ${job.is_active ? 'text-green-600' : 'text-gray-400'}`} />
                  </button>
                  <button onClick={() => deleteJob(job.id)} className="p-2 border border-gray-300 rounded-lg hover:bg-red-50">
