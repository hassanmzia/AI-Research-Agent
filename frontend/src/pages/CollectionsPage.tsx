    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">Organize your favorite papers into curated collections</p>
        </div>
        <button
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5 space-y-3">
          ...
          <div className="flex gap-2">
            <button type="submit" className="flex-1 sm:flex-none px-4 py-2.5 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700">
              Create
            </button>
            <button type="button" onClick={() => setShowForm(false)} className="flex-1 sm:flex-none px-4 py-2.5 border border-gray-300 rounded-lg text-sm hover:bg-gray-50">
              Cancel
            </button>
          </div>
        </form>
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="font-semibold text-gray-900 text-sm sm:text-base truncate">{col.name}</h3>
                    {col.description && <p className="text-xs sm:text-sm text-gray-500 mt-0.5 truncate">{col.description}</p>}
                  </div>
                </div>
                <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                  <span className="text-xs sm:text-sm text-gray-500">{col.papers_count}</span>
                  <span className="hidden sm:inline text-xs px-2 py-0.5 bg-gray-100 rounded text-gray-600">
                  ...
                  {loadingDetail ? (
                    ...
                    <div className="divide-y divide-gray-50">
                      {expandedCollection.papers.map((paper) => (
                        <div key={paper.id} className="flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 hover:bg-gray-50">
                          <Link to={`/papers/${paper.id}`} className="flex-1 min-w-0 mr-3">
                            <p className="text-xs sm:text-sm font-medium text-gray-900 hover:text-primary-600 truncate">{paper.title}</p>
                            <div className="flex items-center gap-2 mt-0.5 text-xs text-gray-500">
                              <span>{paper.source}</span>
                              {paper.published_date && <span className="hidden sm:inline">{new Date(paper.published_date).toLocaleDateString()}</span>}
                            </div>
                          </Link>
